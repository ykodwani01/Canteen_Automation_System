...
class ModelsTestCase(TestCase):
    def setUp(self):
        ...
        self.profile = Profile.objects.create(user=self.user, type='Customer', name='Test Customer', contact_number='1234567890')
        self.canteen_owner_profile = Profile.objects.create(user=User.objects.create(username='canteen_owner'), type='Canteen', name='Canteen Owner')
        ...
        self.item = items.objects.create(canteen=self.canteen, name='Test Item', desc='Test Description', price=10, available=True)
        self.order = orders.objects.create(order_cust=self.customer, order_canteen=self.canteen, total_amount=10, status='AddedToCart')
        self.order_quantity = orderquantity.objects.create(order_id=self.order, item_id=self.item, quantity=2)
        self.default_order_quantity = orderquantity.objects.create(order_id=self.order, item_id=self.item)
    ...
    def test_order_status_choices(self):
        valid_statuses = [status[0] for status in orders._meta.get_field('status').choices]
    ...
    def test_orderquantity_default_quantity(self):
        default_quantity_order_quantity = orderquantity._meta.get_field('quantity').default
        self.assertEqual(self.default_order_quantity.quantity, default_quantity_order_quantity)
